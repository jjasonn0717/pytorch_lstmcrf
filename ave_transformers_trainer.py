...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader):
    ...
    model = AVETransformersCRF(config)
    ...
    model_folder = config.model_folder[:-1] if config.model_folder[-1] == '/' else config.model_folder
    #res_folder = "results"
    #res_folder = model_folder
    if os.path.exists(model_folder):
        raise FileExistsError(
            f"The folder {model_folder} exists. Please either delete it or create a new one "
            f"to avoid override.")
    model_path = f"{model_folder}/model/lstm_crf.m"
    config_path = f"{model_folder}/model/config.conf"
    res_path = f"{model_folder}/results/test_outputs.jsonl"
    print("[Info] The model will be saved to: %s.tar.gz" % (model_folder))
    os.makedirs(os.path.join(model_folder, 'model'), exist_ok= True) ## create model files. not raise error if exist
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        with tqdm(enumerate(train_loader, 1), desc="--training batch", total=len(train_loader)) as tepoch:
            for iter, batch in tepoch:
                optimizer.zero_grad()
                loss, batch_max_scores, batch_max_ids = model(words=batch.input_ids.to(config.device),
    ...
    with tarfile.open(f"{model_folder}/model.tar.gz", "w:gz") as tar:
        tar.add(f"{model_folder}/model", arcname='model')
    if config.use_s3:
        s3_path = f"slin/catalog/models/{os.path.basename(model_folder)}/model.tar.gz"
        s3_upload_file(f"{model_folder}/model.tar.gz", s3_path)
...
def main():
    ...
    if opt.mode == "train":
        ...
        if opt.use_s3:
            ...
            file_path = os.path.join(temp_dir, os.path.basename(opt.model_folder))
            s3_download_file(opt.model_folder, file_path)
            with tarfile.open(file_path, "r:gz") as tar:
                tar.extractall(path=temp_dir)
            folder_name = os.path.join(temp_dir, 'model')
        else:
            folder_name = opt.model_folder
